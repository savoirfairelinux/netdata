...
RRDSET* rrdset_init(RRD_MEMORY_MODE memory_mode, const char *id, const char *fullid, const char *filename)
{
...
{
    return rrdset_init(RRD_MEMORY_MODE_MAP, id, fullid, filename);
}

RRDSET* rrdset_init_ram(const char *id, const char *fullid, const char *filename)
{
    return rrdset_init(RRD_MEMORY_MODE_RAM, id, fullid, filename);
}

RRDSET* rrdset_init_save(const char *id, const char *fullid, const char *filename)
{
    return rrdset_init(RRD_MEMORY_MODE_SAVE, id, fullid, filename);
}


RRDDIM* rrddim_init(RRDSET *st, RRD_MEMORY_MODE memory_mode, const char* filename, int map_mode)
{
...
{
    return rrddim_init(st, RRD_MEMORY_MODE_MAP, filename, MAP_SHARED);
}
RRDDIM* rrddim_init_ram(RRDSET *st, const char *id, const char *filename)
{
    return rrddim_init(st, RRD_MEMORY_MODE_RAM, NULL, MAP_PRIVATE);
}
RRDDIM* rrddim_init_save(RRDSET *st, const char *id, const char *filename)
{
    return rrddim_init(st, RRD_MEMORY_MODE_SAVE, filename, MAP_PRIVATE);
}
...
storage_number rrddim_query_next_metric(struct rrddim_query_handle *handle, time_t *current_time) {
    RRDDIM *rd = handle->rd;
    struct mem_query_handle* h = handle->handle;
