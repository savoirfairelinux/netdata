...
RRDHOST *rrdhost_create(const char *hostname,
    ...
    // Create engine
    host->rrdeng_ctx = engine_new(engine_get(host->rrd_memory_mode), host, is_legacy);
...
int rrd_init(char *hostname, struct rrdhost_system_info *system_info) {
    ...
    for (STORAGE_ENGINE* eng = engine_foreach_init(); eng; eng = engine_foreach_next(eng)) {
        if (!eng->instance_per_host) {
            STORAGE_ENGINE_INSTANCE* ret = engine_new(eng, localhost, false);
            if (!ret) {
                error(
