 ...
 */
int metalog_init(struct rrdengine_instance *rrdeng_parent_ctx, struct mongoengine_instance *mongoeng_parent_ctx)
{
    ...
    if (rrdeng_parent_ctx) {
        rrdeng_parent_ctx->metalog_ctx = ctx;
        ctx->rrdeng_ctx = rrdeng_parent_ctx;
    }
    if (mongoeng_parent_ctx) {
        mongoeng_parent_ctx->metalog_ctx = ctx;
        ctx->mongoeng_ctx = mongoeng_parent_ctx;
    }
