...
static void delete_old_data(void *arg)
{
    ...
    for (extent = datafile->extents.first ; extent != NULL ; extent = next) {
        ...
        for (i = 0 ; i < count ; ++i) {
            ...
            if (unlikely(can_delete_metric && ctx->parent.metalog_ctx->initialized)) {
                /*
                 ...
                 */
                metalog_delete_dimension_by_uuid(ctx->parent.metalog_ctx, &metric_id);
            }
        }
        next = extent->next;
        freez(extent);
...
void timer_cb(uv_timer_t* handle)
{
    ...
    uv_stop(handle->loop);
    uv_update_time(handle->loop);
    if (unlikely(!ctx->parent.metalog_ctx->initialized))
